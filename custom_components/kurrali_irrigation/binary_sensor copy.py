...
async def async_setup_entry(hass, entry: config_entries.ConfigEntry, async_add_devices):
    ...
    for zone in range(available_stations.stations.count):
        if available_stations.stations.active(zone + 1):
            name = entry.data["stations"][zone]
            _LOGGER.info("Setting up rainbird station %s with name %s" % (str(zone + 1), name))
            entities.append(
...
class RainbirdStationEntity(CoordinatorEntity[RainbirdUpdateCoordinator[States]], BinarySensorEntity):
    ...
    @property
    def icon(self):
        return self.is_on if ICON_ZONE_ON else ICON_ZONE_OFF
